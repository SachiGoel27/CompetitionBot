...
# [FIXED] True server telemetry used for SPACE. 
# PRIMED and CARPET assumed to be uniform static to prevent Ghost Rat hallucinations.
NOISE_PROBS = {
    Cell.BLOCKED: (0.5, 0.3, 0.2),
    Cell.SPACE: (0.53, 0.24, 0.23),
    Cell.PRIMED: (0.50, 0.25, 0.25),
    Cell.CARPET: (0.50, 0.25, 0.25),
}
...
class PlayerAgent:
    ...
    def heuristic(self, board, turns_left):
        ...
        def evaluate_line(start_x, start_y, end_x, end_y, length, is_horizontal):
            ...
            # [FIXED] Strictly cap the potential value so the bot never refuses to carpet.
    ...
    def quiescence_search(self, board, alpha, beta, is_maximizing, start_time, time_limit, turns_left):
        """
        Stockfish Improvement: Evaluates only highly volatile moves (captures/carpets)
        at leaf nodes to prevent the Horizon Effect.
        """
        ...
        if is_maximizing:
            ...
            # [FIXED] Must reverse perspective FIRST to safely get opponent's moves
    ...
    def minimax(self, board, depth, alpha, beta, is_maximizing, start_time, time_limit, turns_left):
        ...
        if is_maximizing:
            ...
            def move_priority(m):
                if m == hash_move: return float('inf') # Stockfish Hash Move Ordering
            ...
            bSearchPv = True # Principal Variation Search Flag
            for move in valid_moves:
                next_board = board.forecast_move(move, check_ok=False)
                if next_board is None: continue
                
                if bSearchPv:
                    # Full window search for the presumed best move
                    ...
                    if alpha < eval_score < beta: # If it failed high, research with full window
            ...
            for move in valid_moves:
                ...
                if bSearchPv:
                    ...
                    # Null window search for opponent
    ...
    def play(self, board, rat_samples, time_left_func):
        ...
        # [FIXED] Force the threshold to obey our actual weights so we catch the rat!
