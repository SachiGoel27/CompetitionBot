...
class RatBelief:
    ...
    def reset_and_headstart(self, is_initial_spawn=False, board=None):
        if is_initial_spawn:
            # Use the cached 1000-turn random walk only for the start of the game
            self.belief = self.stationary_belief.copy()
        else:
            # Mid-game catch: The rat respawns uniformly. 
            # We wipe the belief and distribute probability equally across valid tiles.
            self.belief = np.zeros(64)
            valid_cells = 0
...
class PlayerAgent:
    ...
    def check_rat_resets(self, board):
        ...
        if op_guess is not None:
            if op_correct:
                # [FIXED] Pass the board and initial spawn flag to the opponent's catch logic
                self.hmm.reset_and_headstart(is_initial_spawn=False, board=board)
            else:
    ...
    def commentate(self):
        ...
        return f"Depth: {self.last_search_depth} | Ghost Rats: {ghosts} ({ghost_rate:.1f}%) | Max False Conf: {max_conf:.2f}"