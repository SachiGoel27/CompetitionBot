...
class PlayerAgent:
    def __init__(self, board, transition_matrix, time_left_func):
        ...
        # DEFAULT WEIGHTS (These will be overwritten by the cluster tuner)
        self.weights = {
            "prime_exp": 1.5,
            "prime_mult": 0.4,
            "center_bonus": 0.5,
            "rat_confidence_threshold": 0.0 # EV threshold modifier
        }
        
        # Read mutated weights from the environment if they exist
    ...
    def heuristic(self, board):
        ...
        # 1. Parameterized Carpet Maximization
        for y in range(8):
            row_chain = 0
            for x in range(8):
                if board.get_cell((x, y)) == Cell.PRIMED:
                    row_chain += 1
                    # Use the tuned weights!
        ...
        # 2. Add a new metric to tune: Center Control
        center_bonus = 0
