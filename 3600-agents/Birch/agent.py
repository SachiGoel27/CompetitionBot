...
class PlayerAgent:
    ...
    def heuristic(self, board):
        ...
        def evaluate_line(line_coords):
            ...
            # Massive base value to force priming when the board is empty
            base_val = 0.6 if length == 1 else max(0, actual_points * 0.80)
                
            my_dist = min([abs(my_x - px) + abs(my_y - py) for px, py in line_coords])
            en_dist = min([abs(en_x - px) + abs(en_y - py) for px, py in line_coords])
            
            if my_dist < en_dist:
                # We own it! Reward being further away from the enemy.
                return base_val + (en_dist - my_dist) * 0.15
            elif en_dist < my_dist:
                # Enemy owns it! Apply penalty, but REDUCE penalty as we get closer.
                # This creates a "gravity well" pulling us to steal it.
                return (-base_val * 1.5) + ((15 - my_dist) * 0.25)
            else:
                # Contested. We want to roll this immediately.
                return base_val * 0.5
    ...
    def play(self, board, rat_samples, time_left_func):
        ...
        # If there is a massive carpet available, TAKE IT IMMEDIATELY.
        ...
        if carpet_moves:
            ...
            # Threshold of 3 ensures we capture solid points without overthinking.
            if best_carpet.roll_length >= 3: 
                return best_carpet
        ...
        try:
            while True:
                # Stop initiating deeper levels if we are running low on the allocated time.
        ...
        adjusted_threshold = self.weights["rat_confidence_threshold"]
        if ev_search > move_delta + adjusted_threshold and ev_search > 0: 
            return Move.search(best_rat_loc)
